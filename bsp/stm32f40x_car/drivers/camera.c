...
/*********************************************************************************
  *函数名称:u16 Cam_Flash_SearchPic(MYTIME *start_time,MYTIME *end_time,Style_Cam_Requset_Para *para,u8 *psrc)
...
#define CAM_START	0x40000
#define CAM_SECTORS 200
#define CAM_END		( CAM_START + CAM_SECTORS * 4096 )

TypeDF_PackageInfo picinfo_curr = { CAM_START, 0, 0 };

typedef uint8_t ( *VisitFunc )( uint32_t addr, TypeDF_PackageHead *phead,void *ctx );


/*查找最近的图片*/
uint8_t cb_pic_init( uint32_t addr, TypeDF_PackageHead *phead,void *data)
{
	if(phead->Data_ID>picinfo_curr.Data_ID)		/*查找最大*/
	{
		picinfo_curr.Address	= addr;
		picinfo_curr.Len		= phead->Len;
		picinfo_curr.Data_ID	= phead->Data_ID;
	}	
	return 0;	/*继续查找*/
}

/*查找指定ID的图片*/
uint8_t cb_pic_findid( uint32_t addr, TypeDF_PackageHead * phead,void *data )
{
	if(phead->Data_ID==*(u32*)data)
	{
		rt_kprintf("find %d (%08x)\r\n",phead->Data_ID,addr);
		return 1;	/*停止查找*/
	}
	return 0;
}
...
/*迭代所有记录*/
uint8_t camera_foreach( MYTIME start, MYTIME end, uint8_t chn, uint8_t evt, VisitFunc visit, void *data )
{
	uint32_t			i, addr, addr_next;
	uint8_t				sector;
	TypeDF_PackageHead	pichead;
	uint8_t				ret;

	MYTIME				time_curr;

	rt_sem_take( &sem_dataflash, RT_TICK_PER_SECOND * 2 );
	for( sector = 0; sector < CAM_SECTORS; sector++ ) /*就是一个计数器*/
	{
		addr = picinfo_curr.Address;
		sst25_read( addr, (uint8_t*)&pichead, sizeof( TypeDF_PackageHead ) );
		...
		if( strncmp( pichead.Head, CAM_HEAD, strlen( CAM_HEAD ) ) != 0 )
		{
			continue;
		}

		if( ( pichead.State & 0x01 ) == 0 ) ///查看该图片是否被删除
		{
			continue;
		}
		if( ( chn != 0xff ) && ( pichead.Channel_ID != chn ) )
		{
			continue;
		}
		if( ( evt != 0xff ) && ( pichead.TiggerStyle != evt ) )
		{
			continue;
		}
		time_curr = Times_To_LongInt( &pichead.Time );

		if( ( time_curr >= start ) && ( time_curr <= end ) ) ///比较时间是否在范围
		{
			ret = visit( addr,(void*)&pichead,data );
			if( ret )
			{
				break;
			}
...
void Cam_Device_init( void )
{
	...
	///初始化flash参数
	//Cam_Flash_InitPara( 0 );
	camera_foreach( 0, 0xFFFFFFFF, 0xFF, 0xFF, cb_pic_init,RT_NULL);
