...
void sms_encode_pdu_7bit( char* info )
{
	...
	for( i = 0; i < len - 1; ++i )
	{
		c		= ( ( pSrc[i] & BITMASK_7BITS ) >> ( carry_on_bits - 1 ) ) | ( ( pSrc[i + 1] & BITMASK_7BITS ) << ( 8 - carry_on_bits ) );
		*pDst++ = ASC[c >> 4];
		*pDst++ = ASC[c & 0xF];
		carry_on_bits++;
		if( carry_on_bits == 8 )
	...
	if( i <= len )
	{
		c		= ( pSrc[i] & BITMASK_7BITS ) >> ( carry_on_bits - 1 );
		*pDst++ = ASC[c >> 4];
		*pDst++ = ASC[c & 0xF];
	}
...
//将每个ascii8位编码的Bit8去掉，依次将下7位编码的后几位逐次移到前面，形成新的8位编码。
u16 gsmencode7bit( u8* pSrc, u8* pDst, u16 nSrcLength )
{
	u16 nSrc;
	u16 nDst;
	u16 nChar;
...
void sms_encode_pdu_7bit( char* info )
{
	...
	*pDst			= 0;
	...
	rt_kprintf( "\nSMS>encode 7BIT(%d):%s",strlen(buf), buf );
	printf("\nAT+CGMS=%d",(strlen(buf)-3)>>1);
	//sms_tx(buf,(nDst+count+10)>>1);
}
#else
...
/*直接在info上操作有误?*/
    ...
    *pdst=0;
	...
	if( ( dcs & 0x0c ) == GSM_7BIT )
	{
		decode_msg_len = gsmdecode7bit( tmpbuf, decode_msg, strlen(tmpbuf));
	}else if( ( dcs & 0x0c ) == GSM_UCS2 )
	{
		decode_msg_len = gsmdecodeucs2( tmpbuf, decode_msg, msg_len );
	}else
...
/*tmpbuf用作应答的信息 TW703#*/
	...
	if( i > 0x7F ) /*要返回信息,有车牌*/
	{
		sprintf( tmpbuf, "%s#%-7s#%s", "712001", "1234567", decode_msg + 6 );
		sms_encode_pdu_ucs2( tmpbuf );
	}else if( i > 0 )
	{
		sprintf( tmpbuf, "%s#%-7s#%s", "712001", "1234567",  decode_msg + 6 );
...
void main(void)
{
	...
	jt808_sms_rx( s[1], strlen( s[1] ) );
	printf("\r\n");
	jt808_sms_rx( s[0], strlen( s[0] ) );
	getchar();
}
