...
/*疲劳驾驶计时*/
static uint32_t duration_acc_on	= 0;                /*疲劳驾驶时间 acc开*/
static uint32_t duration_acc_off	= 0;                /*疲劳驾驶时间acc关*/
...

{
	...
	*( psrc + length )	= 0;
	...
	{
		gps_sec_count++;
		if( process_rmc( (uint8_t*)psrc ) == 0 )    /*处理正确的已定位RMC信息*/
		{
			process_overspeed();
