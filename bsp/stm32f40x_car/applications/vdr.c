...
struct _sect_info
{
	...
	uint16_t	reccord_size;       /*记录大小*/
	uint8_t		sector_index;       /*当前使用的扇区*/
	uint16_t	record_index;       /*当前扇区内记录的索引*/
} sect_info[8] =
{
	{ '8', VDR_08_SECTORS, 32, 128, VDR_08_SECTORS, 32 },
	{ '9', VDR_09_SECTORS, 6,  680, VDR_09_SECTORS, 6  },
	{ 'A', VDR_10_SECTORS, 32, 128, VDR_10_SECTORS, 32 },
	{ 'B', VDR_11_SECTORS, 32, 128, VDR_11_SECTORS, 32 },
	{ 'C', VDR_12_SECTORS, 32, 128, VDR_12_SECTORS, 32 },
	{ 'D', VDR_13_SECTORS, 32, 128, VDR_13_SECTORS, 32 },
	{ 'E', VDR_14_SECTORS, 32, 128, VDR_14_SECTORS, 32 },
	{ 'F', VDR_15_SECTORS, 32, 128, VDR_15_SECTORS, 32 },
};
...
static uint8_t	vdr_09_sect_index	= 5;                /*在一个sect内的记录索引,每个sector存6个*/
		...
		for( count = 0; count < 6; count++ )                                                /*每扇区6条记录*/
		{
			...
			if( mytime_curr != 0xFFFFFFFF )                                                 /*是有效记录*/
			{
				if( mytime_curr >= mytime_vdr_09 )
				{
					...
					vdr_09_sect_index	= count;
				}
			}
			addr += 680;
		}
	}
	rt_kprintf( "%d>vdr_09 sect=%08x datetime=%08x sect_index=%d\r\n", rt_tick_get( ), vdr_09_sect, vdr_09_time, vdr_09_sect_index );
}
...
void vdr_09_put( MYTIME datetime )
{
	...
	{
		vdr_09_sect_index++;                                        /*要写入新的小时记录每680字节*/
		if( vdr_09_sect_index > 5 )                                 /*每个sector存6个小时 0-5,越过一个扇区*/
		{
			vdr_09_sect_index = 0;
			vdr_09_sect++;
		...
		addr = VDR_09_START + vdr_09_sect * 4096 + vdr_09_sect_index * 680; /*每扇区680字节*/
		PACK_INT( bufhead, ( datetime & 0xFFFFF000 ) );                     /*写入新的记录头*/
		bufhead[4]	= HEX2BCD( YEAR( datetime ) );
		bufhead[5]	= HEX2BCD( MONTH( datetime ) );
		bufhead[6]	= HEX2BCD( DAY( datetime ) );
		bufhead[7]	= HEX2BCD( HOUR( datetime ) );
		bufhead[8]	= 0;
		bufhead[9]	= 0;
		sst25_write_through( addr, bufhead, 10 );
	}
	...
	addr	= VDR_09_START + vdr_09_sect * 4096 + vdr_09_sect_index * 680 + minute * 11 + 10; /*跳到指定的位置,开始有10字节头*/
	sst25_write_through( addr, bufhead, 11 );
	rt_sem_release( &sem_dataflash );
